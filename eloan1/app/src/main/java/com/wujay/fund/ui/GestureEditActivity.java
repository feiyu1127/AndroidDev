...
public class GestureEditActivity extends Activity implements OnClickListener {
	...
	private String mSecPassword = null;
			...
			public void onGestureCodeInput(String inputCode) {
				...
				if (mIsFirstInput) {
					...
					if (inputCode.equals(mFirstPassword)) {
						mSecPassword=inputCode;
						//	Toast.makeText(GestureEditActivity.this, "设置成功"+mSecPassword, Toast.LENGTH_SHORT).show();
						mGestureContentView.clearDrawlineState(0L);
				//把密码存进去 	
						if (sp0 == null) {
						...
						edit.putString("Pwd", mSecPassword);
						//首先判断登陆信息是否为空
						if(Contents.response != null && Contents.response.getResult() != null)
							edit.putString("Name", Contents.response.getResult().getCipNamecn());
						else//信息为空，设置其name为空字符串  TODO 此处应斟酌应该做什么处理更正确
							edit.putString("Name", "");
						edit.commit();
						
						FileUtil f=new FileUtil();
						try {
							f.writeSDFile(filename, mSecPassword);
						} catch (IOException e) {
							e.printStackTrace();
						}
