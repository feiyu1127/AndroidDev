...
public class XiaoJinYingZhangDanActivity extends BaseActivity {
	...
	private XiaoJinYingZhangDanMingXiListAdapter nadapter;

	private XiaoJinYingZhangDanMingXiList_Weichu_Adapter weichuAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.xiaojinyin_zhangdan_activity);
		 ...
		 * 
		 * @Override public void moveToDest(int currid) { //
	...
	private void initview() {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// 选中项
				if (currentPosition == position) { // 如果重复点击 关闭
					currentPosition = -1;
				} else {
					currentPosition = position;
				}

				if (nadapter != null) { // 已出帐单适配器
					nadapter.notifyDataSetChanged();
				}
				if (weichuAdapter != null) { // 未出帐单适配器
		...
		backzhangdan.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// 回退
				Intent mIntent = new Intent(XiaoJinYingZhangDanActivity.this,
	...
	/**
	 * 描述: 联网请求数据 </br> 开发人员：Weiyb</br> 创建时间：2015-7-19</br>
	 */
	private void initData() {
		if (zhangdanleixing.equals("1")) {// 已选时间区间
											// 已出帐单查询
			new riqichaxunzhangdanAsyncTask().execute(
					Contents.xiaoChaXunResult2.getCardNo(), kaishi, jieshu);
	...
	class riqichaxunzhangdanAsyncTask extends AsyncTask<String, Object, Object> {
		...
		protected Object doInBackground(String... params) {
			...
			HttpHelper httpHelper = HttpHelper
					.getInstance(XiaoJinYingZhangDanActivity.this);
			...
			return httpHelper.post(ContantsAddress.shijianzhangdan, arg,
					XiaoJinYingMingXiresult.class);
		}
	...
	// 未出帐单查询接口
	class riqichaxunzhangdanAsyncTask2 extends
		...
		protected Object doInBackground(String... params) {
			...
			return httpHelper.post(ContantsAddress.shijianzhangdan_weichu, arg,
	...
	private void riqizhangdanchaxunResponse(Object result) {
		...
		if (response == null) {
			loginErrorMessage = getString(R.string.xiaojinyin_fuwqqqyc);
			DialogUtil.createDialog(this, 1, loginErrorMessage);
			// mViewPager.setVisibility(View.GONE);
			zhangdanmingxiwenzi.setVisibility(View.VISIBLE);
			zhangdanmingxiwenzi.setText(getString(R.string.xiaojinying_zwzdxx));
			listView.setVisibility(View.GONE);
			return;
		} else {
			if (RET_CODE == response.getCode()) {

				// Toast.makeText(XiaoJinYingZhangDanActivity.this, "提交成功" +
				// response.getMsg(),
				...
				// 获取未出帐单数据
				new riqichaxunzhangdanAsyncTask2()
						.execute(Contents.xiaoChaXunResult2.getCardNo());
				// "9999019000600023");
				...
				// Toast.makeText(XiaoJinYingZhangDanActivity.this,
	...
	/**
	 * 描述:处理未出帐单返回结果 </br> 开发人员：weiyb</br> 创建时间：2015-7-7</br>
	 * 
	 * @param result
	 */
	private void riqizhangdanchaxunResponse2(Object result) {
		...
		if (response == null) {
			loginErrorMessage = getString(R.string.xiaojinyin_wczdewqqqyc);
			DialogUtil.createDialog(this, 1, loginErrorMessage);
			// mViewPager.setVisibility(View.GONE);
			zhangdanmingxiwenzi.setVisibility(View.VISIBLE);
			zhangdanmingxiwenzi.setText(getString(R.string.xiaojinying_zwwczdxx));
			listView.setVisibility(View.GONE);
			return;
		} else {
			if (RET_CODE == response.getCode()) {
				...
				// 未出帐单适配器
				weichuAdapter = new XiaoJinYingZhangDanMingXiList_Weichu_Adapter(
						mWeichList, XiaoJinYingZhangDanActivity.this);
	...
	/**
	 * 描述:处理装上部横向列表 </br> 开发人员：weiyb</br> 创建时间：2015-7-7</br>
	 * 
	 * @param mingxilist2
	 */
	@SuppressLint("InflateParams")
	@SuppressWarnings("unused")
	private void initScrollView(List<XiaoJinYingMingXiresult2> mingxilist2,
			Myresult mMyresult) {
		...
		if (mingxilist2 != null) {
			for (int i = 0; i < mingxilist2.size(); i++) {
				...
				TextView mzhangdanriqi = (TextView) mView
						.findViewById(R.id.zhangdanriqi);
				TextView zhangdanjinenum = (TextView) mView
						.findViewById(R.id.zhangdanjinenum);

				// 还款日数字
				TextView huakuanri = (TextView) mView
						.findViewById(R.id.huakuanri);
				// 还款日LiearLayout
				LinearLayout huakuanri_ll = (LinearLayout) mView
						.findViewById(R.id.huakuanri_ll);
				huakuanri_ll.setVisibility(View.VISIBLE);

				TextView zhuidijine = (TextView) mView
						.findViewById(R.id.zhuidijine);

				TextView zhangdanbishu = (TextView) mView
						.findViewById(R.id.zhangdanbishu);
				TextView zhangdanriqi = (TextView) mView
						.findViewById(R.id.zhangdanriqi);
				...
				if (mingxilist2.get(i).getPsBillDt() != null) {
					String[] array = mingxilist2.get(i).getPsBillDt()
							.split("-");
					...
					if (array.length >= 3) {
						...
						zhandanqi = "(" + String.valueOf(p_month) + "."
				...
				// 帐单日期
				zhangdanriqi.setText(mingxilist2.get(i).getPsBillDt() + getString(R.string.xiaojinyig_tzd)
						+ zhandanqi);
		...
		if (mMyresult != null) {
			...
			loginErrorMessage = getString(R.string.xiaojinying_zwzdxx);// "暂无账单信息!"
			DialogUtil.createDialog(this, 1, loginErrorMessage);
		}
	...
	/**
	 * 描述:添加未出帐单项 </br> 开发人员：weiyb</br> 创建时间：2015-7-8</br>
	 * 
	 * @param mMyresult
	 */
	@SuppressWarnings("unused")
	...
	private void addWeichu(Myresult mMyresult) {
		...
		TextView zhangdanjinenum = (TextView) mView
				.findViewById(R.id.zhangdanjinenum);
		TextView zhangdanjine = (TextView) mView
				.findViewById(R.id.zhangdanjine);
		TextView zhangdanbishu = (TextView) mView
				.findViewById(R.id.zhangdanbishu);
		TextView zhangdanriqi = (TextView) mView
				.findViewById(R.id.zhangdanriqi);

		TextView zhangdanjinerenminbi = (TextView) mView
				.findViewById(R.id.zhangdanjinerenminbi);
		// 还款日LiearLayout
		LinearLayout huakuanri_ll = (LinearLayout) mView
				.findViewById(R.id.huakuanri_ll);
		...
		if (mingxilist.size() > 0) {
			shangqizhangdanjin.setText(getString(R.string.xiaojinyin_rmb)
					+ mingxilist.get(mingxilist.size() - 1).getCurAddAmt());
		}

		// 帐单金额

		if (mMyresult.getTotalAmt() != null) {
			zhangdanjinerenminbi.setText(getString(R.string.xiaojinyin_rmb));
		...
		// 帐单日期
		zhangdanriqi.setText(getString(R.string.xiaojinying_wczd));
		// 帐单金额
		...
		zhangdanmingxiwenzi.setText(getString(R.string.xiaojinyin_wczdmx));
	...
	// 账单详情接口
	class zhangdanxiangqingResponseAsyncTask extends
			AsyncTask<String, Object, Object> {
		...
		protected Object doInBackground(String... params) {
			...
			return httpHelper.post(ContantsAddress.MINI_BILL, arg,
					XiaoJinYingZhangDanXiangQing.class);
		}
	...
	/**
	 * 描述: 已出帐单详情 </br> 开发人员：weiyb</br> 创建时间：2015-7-7</br>
	 * 
	 * @param result
	 */
	private void zhangdanxiangqingResponse(Object result) {
		...
		if (response == null) {
			...
			if (RET_CODE.equals(response.getCode())) {
				...
				zhangdanmingxiwenzi.setText(getString(R.string.xiaojinying_zdmx));
				nadapter = new XiaoJinYingZhangDanMingXiListAdapter(
						RealitionInfo99, XiaoJinYingZhangDanActivity.this);
				listView.setAdapter(nadapter);
			} else {
				Toast.makeText(XiaoJinYingZhangDanActivity.this,
						getString(R.string.xiaojinying_ts) + response.getMsg(),
	...
	/**
	 * 描述:页面改变时调用 </br> 开发人员：Weiyb</br> 创建时间：2015-7-19</br>
	 * 
	 * @param position
	 */
	private void onPagerChenge(int position) {
		...
		if (position > -1) {
			...
			if (mingxilist != null) { // 已出帐单不为空的情况下

				if (position == mingxilist.size()) { // 未出帐单
					...
					if (mingxilist.size() > 0) {
						shangqizhangdanjin.setText(getString(R.string.xiaojinyin_rmb)
								+ mingxilist.get(position - 1).getCurAddAmt());
					}

				} else if (position < mingxilist.size()) {// 已出帐单

					// 重置选中项
					currentPosition = -1;
					String st_date = mingxilist.get(position).getPsBillDt()
							.toString();
					// new
					// zhangdanxiangqingResponseAsyncTask().execute("9999010000096118",
					// st_date);
					new zhangdanxiangqingResponseAsyncTask().execute(
							Contents.xiaoChaXunResult2.getCardNo(), st_date);

					if (position >= 1) {

						if (mingxilist.get(position - 1).getLastBillAmt() != null) {
							shangqizhangdanjin.setText(getString(R.string.xiaojinyin_rmb)
									+ mingxilist.get(position - 1)
											.getLastBillAmt());
						} else {
					...
					if (position >= 1) {
						// shangqizhangdanjin.setText("¥ " +
						// mingxilist.get(currid - 1).getLastBillAmt() + "");
