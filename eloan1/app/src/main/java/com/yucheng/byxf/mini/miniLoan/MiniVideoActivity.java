...
public class MiniVideoActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.mini_transcribevideo_activity);
		initView();
		audio();
	}
	private void initView() {
		...
		bt_next.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (checkbox_next.isChecked() && !isplay) {
					...
					if (!isplay) {
						Toast.makeText(MiniVideoActivity.this, "请确认上述条款",
								Toast.LENGTH_SHORT).show();

					} else {
						Toast.makeText(MiniVideoActivity.this, "请听完整录音",
								Toast.LENGTH_SHORT).show();
					}
				}
		...
		String contents = Contents.poetry_str + "\n本人"
				+ Contents.response.getResult().getCipNamecn()
				+ ",同意上述条款。";
	...
	public void onClick(View v) {
		...
		if (v == startButton) {
			if (!mIsRecording) {
				...
				MiniApplyInfo.MninvideoPath = "Minieloanvideo.3gp";
			...
			if (quit.getText().toString().equals("重录")) {
				...
				stopmediaRecorder();
				finish();
				MiniApplyInfo.MninvideoPath = null;
			}
		} else if (v == sure) {
