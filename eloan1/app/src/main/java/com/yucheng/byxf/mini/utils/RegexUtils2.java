...
public class RegexUtils2 {
	// 匹配身份证号码
//	private static final String idcard = "\\d{15}|\\d{18}";
	...
	// 密码
	private static final String user_password = "[a-zA-Z][a-zA-Z0-9]{7,19}";
	// 验证是否都为数字
	private static final String a = "[0-9]*";
	
	// 验证是否都为字母
	private static final String user_password1 = "[A-Za-z]+";
	...
	public static boolean negativeInteger(String value) {
		return isNotNull(value) && value.matches(negativeInteger);
	}
	...
	public static Result user_password(String label, String value) {
		boolean match = user_password(value);
		String msg = null;
		if (!match) {
			msg = label + "不能小于8位，不能大于30位";
		}
		return new Result(match, msg);
	}
	...
	public static boolean ChEngNum(String value) {
		return isNotNull(value) && value.matches(ChEngNum);
	}
	//中文+ 英文+数子
	...
	public static void main(String[] args) {
		// 姓名为必填项
		// System.out.println(required("姓名",null));
		// System.out.println(required("姓名",""));
		// System.out.println(required("姓名","2323"));
		// 身份证为必填项,且必须符合格式
		// System.out.println(idcard("身份证号码", null));
		// System.out.println(idcard("身份证号码", "110110198709082334"));
		// 电子邮箱可以为空,但是只要输入了就必须符合电子邮箱格式
		// String myEmail = "1111.com";
		// if (myEmail != null) {
		// // System.out.println(email("电子邮箱", myEmail));
		// } else {
		// // System.out.println("电子邮箱为空时，也可以，do nothing....");
		// }

		String test = "a00a1AAA1";
		if (test != null) {
			System.out.println(test.matches(user_password));
		} else {
			System.out.println("电子邮箱为空时，也可以，do nothing....");
		}
	}
